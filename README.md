...
# Create a directory path for Nuscenes:
...
# - File blobs of 85 scenes, parts 1-10 [US, Asia] (~284.17 GB total)
...
# Adjust the GPUs configuration:
# Change GPUS: [0,1,2,3,4] to GPUS: [0] for single GPU (RTX 4090).
# Optionally, adjust EPOCHS to 20 (default setting).

## Training Modules

### 1. (Recommended) Perception Module Pretraining
bash scripts/train_perceive.sh ${configs} ${dataroot}

### 2. (Optional) Prediction Module Training
# This step is for training purposes, not needed for end-to-end:
bash scripts/train_prediction.sh ${configs} ${dataroot} ${pretrained}

### 3. Entire Model End-to-End Training
bash scripts/train_plan.sh ${configs} ${dataroot} ${pretrained}

### 4. Execute Perception Training
...
## Set Up on a Second PC for Efficiency
...
# Due to the superior processing power of the RTX 4090, use it for planning:
